...
const CoverForm = ({ coverImage, title, description }) => {
	return (
		<div className="p-5 bg-gray-400 rounded-t-md h-[150px] ">
			{coverImage ? (
				<div>
					<img src={coverImage} className="w-full h-12 rounded-md" alt="" />
				</div>
			) : (
				<div className="w-full h-12 rounded-md bg-purple-500"></div>
			)}

			<div className="bg-gray-600 border-t-8 cover-form-header mt-2 pl-4 border-t-purple-950 rounded-md px-2 py-1">
